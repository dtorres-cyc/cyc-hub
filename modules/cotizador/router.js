...
router.get('/api/status', async (req, res) => {
  res.json({
    ...
    next_number:  await db.nextQuoteNumber(),
  });
});
...
router.delete('/api/products/:pid', async (req, res) => {
  await db.deleteProduct(parseInt(req.params.pid));
  res.json({ ok: true });
});

// ── Cotizaciones ──────────────────────────────────────────────────────────────
router.get('/api/quotes', async (req, res) => {
  const quotes = (await db.getAllQuotes()).map(q => ({
    ...q,
...
router.post('/api/generate', async (req, res) => {
  try {
    ...
    const numero = await db.nextQuoteNumber();
    ...
    const rowId = await db.saveQuote(dbData);
    ...
    if (google.isReady()) {
      try {
        ...
        await db.updateDriveInfo(rowId, driveId, driveUrl);
      } catch (e) {
      ...
      if (sendMail && emailTo) {
        try {
          ...
          await db.updateEmailSent(rowId);
          emailSent = true;
        } catch (e) {
...
router.post('/api/preview', async (req, res) => {
  try {
    const d = req.body;
    const pdfData = {
      numero:       await db.nextQuoteNumber(),
